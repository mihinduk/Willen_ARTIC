#!/usr/bin/env python3
"""
Parse MMseqs2 results and classify hits.
- Non-COVID: classify by taxonomy (human/bacterial/other), create FASTAs
- COVID hits: map to genes and create TSV

Usage:
    python3 parse_mmseqs_results.py mmseqs_results.txt reads.fasta sam_metadata.tsv output_prefix
"""
...
def classify_taxonomy(sciname):
    ...
    if 'severe acute respiratory syndrome' in sciname_lower:
        return 'covid'
    if 'coronavirus 2' in sciname_lower:
        return 'covid'
    ...
    # Viral (non-COVID)
    if 'virus' in sciname_lower or 'phage' in sciname_lower or 'viral' in sciname_lower:
        return 'viral'
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python3 parse_mmseqs_results.py mmseqs_results.txt reads.fasta sam_metadata.tsv output_prefix")
        print("\nMMseqs2 results should be in BLAST-like format with taxonomy:")
        print("  qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore staxids sscinames")
        sys.exit(1)

    mmseqs_file = sys.argv[1]
    fasta_file = sys.argv[2]
    metadata_file = sys.argv[3]
    output_prefix = sys.argv[4]

    print(f"Processing MMseqs2 results: {mmseqs_file}")
    ...
    # Parse MMseqs2 results - collect all hits per read
    read_hits = defaultdict(list)

    with open(mmseqs_file, 'r') as f:
        for line in f:
    ...
    print(f"Loaded MMseqs2 results for {len(read_hits):,} reads\n")
    ...
    ambiguous_reads = []
    top5_agreement = Counter()

    for qseqid, hits in read_hits.items():
        if not hits:
            continue

        # Sort by bitscore (best first)
    ...
    print(f"Processed {len(read_hits):,} reads with MMseqs2 hits\n")
    ...
    with open(covid_tsv, 'w') as f:
        f.write("read_name\tmapped_length\tstart\tstop\tgene_name\tpercent_identity\tevalue\ttop5_all_covid\tnum_mmseqs_hits\n")
